...
def read_punch_file(day, revise_station_name, gs):
    ...
    punch_raw_df['lookup'] = punch_raw_df['']        .str.cat(punch_raw_df['function_name'], sep=', ')        .str.cat(punch_raw_df['function_role'], sep=', ')  # 將Unnamed, function_name, function_role三個欄位合再一起，作為參照
...
def add_data_in_inb(time2, gs, day, month_fullname, inb_pics_file_path_new):
    ...
    docked_df = docked_df[docked_df["DATE"] == day]
